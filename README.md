...
В приложении доступны обзор статуса, проверка файлов и папок, детали срабатываний, локальный карантин с безопасным восстановлением и экспорт JSON, HTML, CSV или SARIF. Анализ работает в фоне и не блокирует окно.

### Windows: один запуск без терминала

После получения исходников дважды щёлкните `Start ArmorAV.cmd` в корне папки. При первом запуске файл сам соберёт self-contained версию Desktop-приложения, а затем откроет ArmorAV. В последующие разы он сразу откроет уже собранный `ArmorAV.Desktop.exe`.

Для первого запуска на компьютере нужен .NET 8 SDK и доступ к NuGet, чтобы получить пакеты Avalonia. После сборки `ArmorAV.Desktop.exe` не требует установленного .NET Runtime.
