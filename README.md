...
Browser Console запускается только на `127.0.0.1`, выбирает свободный локальный порт и открывает браузер автоматически. Проверяемые файлы не отправляются в интернет.

> ArmorAV использует сигнатуры, структурный анализ и эвристики. Это не замена системному антивирусу с real-time protection. Проверяйте срабатывания до удаления файлов. Проект не может гарантировать обнаружение всех угроз, включая неизвестные уязвимости и zero-day-атаки.
...
3. В меню нажмите `2` для Browser Console или `1` для CLI.
4. При первом запуске выбранный режим соберётся локально. Browser Console затем автоматически откроется в браузере.
5. В дальнейшем этот же файл проверит версию и при необходимости пересоберёт только выбранный режим.
...
CLI-исполняемый файл расположен в `artifacts\win-x64\cli\armorav.exe`. Launcher открывает CLI в отдельном окне со справкой. Browser Console можно запускать двойным щелчком или закрепить на панели задач. При запуске он сам открывает браузер с интерфейсом ArmorAV.

Если окно браузера не открылось или страница недоступна, закройте ArmorAV и повторно выберите пункт `2` в `Start ArmorAV.cmd`. При неудачном старте диагностика записывается в `%LOCALAPPDATA%\ArmorAV\armorav-web.log`.
...
- AES-256-GCM карантин с аутентификацией метаданных, атомарной записью, защитой от перезаписи при restore и повторной проверкой SHA-256 перед удалением оригинала.
- Потоковый анализ ZIP с лимитами глубины, записей и распакованных байтов; содержимое записи в памяти ограничено 8 МБ.
- Кэш повторно использует результат только при точном SHA-256 того же содержимого и версии движка.
- Случайный токен локальной сессии и браузерные security headers защищают API от запросов со сторонних сайтов.
...
Start ArmorAV.cmd          меню CLI / Browser Console двойным щелчком в Windows
```
